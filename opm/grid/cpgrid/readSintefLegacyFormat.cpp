    ...
    namespace
    {
                      ...
                      std::vector<std::array<int,8> >& c2p)
        {
            ...
            if (topo_header.compare(correct_header)) {
                ...
                str << "Header of topology file does not match what we expect, file possibly in wrong format.\n"
            ...
            for (int i = 0; i < num_cells; ++i) {
                ...
                for (int j = 0; j < numf; ++j) {
                    ...
                    if (face2cell[face][ind] != -1) {
                        OPM_THROW(std::runtime_error, "Error in creating faces to cells "
                                                      "mapping for face " + std::to_string(face));
                    }
